...
**Status: in progress.** The custom-action seam shipped in slice 406; the context enrichment + first retrofit (Elemental Weapon) shipped in slice 407. Thunder Step + Absorb Elements retrofits, Axis A (effect-lifecycle), and the pack manifest remain. This captures how a content pack ships consumer-supplied *code* (handlers/planners for bespoke mechanics) alongside its JSON, so the engine becomes extensible by data *and* behavior, not data alone. It exists to pin the hard constraints and the public-API contract before each phase, because a plugin API is a long-lived commitment.
...
2. ◐ **Enrich `HandlerContext`** with the curated rules helpers (grow on demand). **Started (slice 407):** added `assertActorCanAct`, `spellSlotsRemaining`, `newEffectInstanceId` for the first retrofit. The save/damage cluster (`computeSavingThrow`, `mitigateDamage`, the fatal chokepoint, spell DC) lands when thunder-step / absorb-elements need it; the surface freezes once all three retrofits are expressible.
3. ◐ **Retro-fit the 3 non-SRD planners** (absorb-elements / thunder-step / elemental-weapon) as reference plugin handlers, proving the API is rich enough to express real spells and removing the spell-named methods from the engine surface. **Elemental Weapon shipped (slice 407):** `planElementalWeapon` + `engine.plan.elementalWeapon` deleted; the behavior now lives in a reference `ActionHandler` ([tests/fixtures/handlers/elemental-weapon.ts](../tests/fixtures/handlers/elemental-weapon.ts)) invoked via `engine.plan.custom`. Thunder Step + Absorb Elements remain (they need the save/damage context cluster).
