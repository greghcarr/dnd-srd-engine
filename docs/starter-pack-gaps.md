...
- `hideous-laughter-active` (Hideous Laughter): empty, but RAW = on a failed WIS save the target has **Prone + Incapacitated** for the duration (with an end-of-turn / on-damage repeat save). The single-`conditionOnFail` save mechanic can't apply two base conditions; needs either multi-condition application or a composite condition expressing Incapacitated (action-blocking allowlist) plus Prone's attack-roll (dis)advantage. The recurring-save half is feasible (spell DC). **Open.**
