...
**Promoted to a permanent audit (slice 363):** [tests/audit/pack-integrity.test.ts](../tests/audit/pack-integrity.test.ts) now asserts "every condition applied by a wired spell either carries effects or is on a documented `EFFECT_LESS_OK` allowlist." The remaining open bugs (now two after slice 366 closed Hideous Laughter and slice 367 closed Bestow Curse's ability arm: `resisted` and `cursed-inert-active`) are allowlisted with their reason; when one is fixed, the allowlist-accuracy check forces it off the list. The same slice added a content-cross-reference guard (every `spellId` / `parentClassId` / `enchantmentDefinitionId` / condition-id reference must resolve to a defined entity), so a renamed or deleted id can no longer leave a silent dangling reference.
