...
**Promoted to a permanent audit (slice 363):** [tests/audit/pack-integrity.test.ts](../tests/audit/pack-integrity.test.ts) now asserts "every condition applied by a wired spell either carries effects or is on a documented `EFFECT_LESS_OK` allowlist." The remaining open bug (now one after slices 366-368 closed Hideous Laughter, Bestow Curse's ability arm, and Bestow Curse's inactive-turn arm: only `resisted` remains) is allowlisted with its reason; when it's fixed, the allowlist-accuracy check forces it off the list. The same slice added a content-cross-reference guard (every `spellId` / `parentClassId` / `enchantmentDefinitionId` / condition-id reference must resolve to a defined entity), so a renamed or deleted id can no longer leave a silent dangling reference.
