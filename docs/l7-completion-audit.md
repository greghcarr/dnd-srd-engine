...
| `npc-caster-bonus-action-groups` | QUIRK | Engine (content+seam) | M | NPC casters that print a second, bonus-action/reaction spell group beyond *Spellcasting*. **Closed so far:** the **Cultist Fanatic** *Spiritual Weapon* 2/Day (slice 815 — an intrinsic Bonus-Action spell on the generic `castSpell` per-day path) and **Misty Step 3/Day** on the **Mage + Archmage** (slice 817 — its dedicated `planMistyStep` gained a `useFreeCast`/per-day branch + granted-spell recognition mirroring the slice-794 machinery, since Misty Step is off the generic cast path). **Still open** (each needs a distinct engine seam): **Divine Aid 3/Day** (Priest) — a *shared* 3/Day pool across Bless/Dispel Magic/Healing Word/Lesser Restoration (`perDayCastsUsed` is per-spell) + the two Action spells cast as a Bonus Action (cast-time override); **Protective Magic 3/Day** (Mage, Archmage) — a Counterspell/Shield reaction needing the monster reaction seam; **Dryad Tree Stride** — a teleport movement ability, not a spell. |
