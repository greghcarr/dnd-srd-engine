...
| `npc-caster-bonus-action-groups` | QUIRK | Engine (content+seam) | M | Several NPC casters print a second spell action — a **bonus-action / reaction group** separate from *Spellcasting*: Mage *Misty Step* 3/Day + *Protective Magic* (Counterspell/Shield reactions), Archmage *Misty Step* 3/Day, Priest *Divine Aid*, Cultist Fanatic *Spiritual Weapon* 2/Day, Dryad *Tree Stride* (BA). A BA-cast spell (Misty Step) can ride a `GrantSpell{perLongRest}` since its casting time *is* the Bonus Action, but the reaction group (Protective Magic) needs the reaction seam, so these are action-economy placement, not the core Spellcasting action. Deferred from slices 794 and 814. |
