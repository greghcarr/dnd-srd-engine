...
| `npc-caster-bonus-action-groups` | QUIRK | Engine (content+seam) | M | NPC casters that print a second, bonus-action/reaction spell group beyond *Spellcasting*. **Partially closed by slice 815** — the **Cultist Fanatic** *Spiritual Weapon* 2/Day is the one item needing no engine work (an intrinsic Bonus-Action spell on the generic `castSpell` per-day path; metering + BA economy fall out of the slice-794 primitive). The rest each need a seam: **Misty Step 3/Day** (Mage, Archmage) routes through the dedicated `planMistyStep`, which consumes a spell *slot* and recognizes only known/prepared spells — a slot-less monster needs a per-day branch + granted-spell recognition there (the earlier "Misty Step rides `GrantSpell{perLongRest}` on `castSpell`" note was wrong; Misty Step is not on the generic path). **Divine Aid 3/Day** (Priest) is a *shared* 3/Day pool across Bless/Dispel Magic/Healing Word/Lesser Restoration (`perDayCastsUsed` is per-spell) and casts the two Action spells as a Bonus Action (cast-time override). **Protective Magic 3/Day** (Mage, Archmage) is a Counterspell/Shield reaction — needs the monster reaction seam. **Dryad Tree Stride** is a teleport movement ability, not a spell. |
| `spiritual-weapon-immediate-attack-action-cost` | DIVERGENCE | Engine | S | Casting Spiritual Weapon **at a target** consumes the Action too, not just the Bonus Action. RAW (spells.md): casting time is a Bonus Action and "you can immediately make one melee spell attack" *as part of that cast* — no separate Action. `cast-spell.ts`'s `consumesImplicitMagicAction` (built for Produce Flame, whose hurl genuinely IS a separate Magic action) over-fires for any BA spell with an attack mechanic + non-instantaneous duration + a target. Surfaced by slice 815 (the Cultist Fanatic grant); affects **every** Spiritual Weapon caster (cleric, player, monster), not just the cultist. Needs a spell-specific flag distinguishing "immediate attack is part of the cast" from "hurl is a separate action." |
