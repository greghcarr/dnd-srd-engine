...
| 5. Build & leveling validation | 11 | 0 | 2 | 4 | Engine |
...
| `npc-caster-sweep-remainder` | QUIRK | Engine (content) | M | The slice 794-795 primitive closed the four named NPC casters, but the same shape recurs on more statblocks: the **Archmage** (full Spellcasting list), each caster's **bonus-action / reaction spell group** (Priest Divine Aid 3/Day, Cultist Spiritual Weapon 2/Day, Mage Misty Step 3/Day + Protective Magic), and the **dragons / giants** whose at-will `GrantSpell` casting (slices 527/529) lacks a `SetSpellcastingProfile`, so their save spells (Fear, etc.) derive the wrong DC. Pure content — author the profile + buckets, no new engine work. |
