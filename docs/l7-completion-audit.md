...
| ID | Sev | Owner | Fix | Finding |
|---|---|---|---|---|
| `multiattack-unpopulated` | **BLOCKER** | Engine (content) | L | Multiattack was authored on only 9/214 statblocks; ~169 in-scope RAW multiattackers (ogre, knight, all young/wyrmling dragons, owlbear, …) did one attack → roughly half their damage. Planner + schema support it. `src/schemas/content/monster.ts`; `src/derive/multiattack.ts`. **Sweep in progress** (SRD-verified via workflow + apply-script, by CR band): **CR 0-1 closed** (slices 789-790: Goblin Boss, Violet Fungus, Sahuagin Warrior — the band is multiattack-poor), **CR 2-5 closed** (slice 791: 64 multiattackers + 88 natural weapons). **Remaining: CR 6-11** (47 statblocks). Follow-up: a focused on-hit-**rider** pass for the ~15 (so far) size/charge-gated grapple/prone riders + the riders needing new condition defs (Bearded Devil `infernal-wound`, Werebear lycanthropy) — deferred from the bulk sweep because the flat extraction schema can't express the gate predicate. |
