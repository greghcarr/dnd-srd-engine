...
| `npc-reaction-discovery` | QUIRK | Engine (seam) | S | Monsters' reaction options aren't surfaced by the reaction-affordance layer. Slice 819 wired the Mage/Archmage *Protective Magic* (Counterspell/Shield) **cast + metering** path, but `engine.query.availableReactions` / `reactionsForTrigger` (slices 763-767) enumerate reactions from class features / known reaction spells, not a monster's effect-stack `GrantSpell` reaction grants — so a consumer driving the Mage must know to offer Counterspell/Shield itself. Additive query-layer work: include granted reaction-castingTime spells (with their per-day pool gate) in the discovery helpers. |
