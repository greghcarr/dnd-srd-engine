...
| `monster-whirlwind-actions` | QUIRK | Engine | M | Split from `monster-onhit-rider-pass` (slice 828). Two save-actions the slice-828 `saveActions` shape doesn't yet cover: the **Air Elemental Whirlwind** (Recharge 4–6; STR DC 13, ≤Medium in the elemental's space, 4d10+2 thunder + push 20 ft + Prone, **half on success**) needs recharge integration (the breath-weapon turn-start path) + forced-push (the existing position-less `ForcedMovement` informational event) layered onto a save-action; and the **Djinni Create Whirlwind** is not a save-or-grapple at all but a **Concentration-sustained conjured 20-ft Cylinder** that moves 20 ft/turn and saves creatures who enter/start in it — a persistent-hazard primitive (closest kin: a summoned AoE), a larger slice. `halfDamageOnSuccess` already ships on `SaveActionSpec` for the Whirlwind half-damage arm. |
