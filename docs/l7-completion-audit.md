...
| `npc-caster-bonus-action-groups` | QUIRK | Engine (content+seam) | M | NPC casters that print a second, bonus-action/reaction spell group beyond *Spellcasting*. **Closed so far:** the **Cultist Fanatic** *Spiritual Weapon* 2/Day (slice 815 — intrinsic Bonus-Action spell on the generic `castSpell` per-day path); **Misty Step 3/Day** on the **Mage + Archmage** (slice 817 — its dedicated `planMistyStep` gained a `useFreeCast`/per-day branch + granted-spell recognition); the **Priest** *Divine Aid* 3/Day (slice 818 — two new GrantSpell fields: `perDayPoolId` shares one N/Day budget across Bless/Dispel Magic/Healing Word/Lesser Restoration by summing their `perDayCastsUsed` counters, no resource seeding needed; `castAsBonusAction` casts the two Action spells as a Bonus Action); and **Protective Magic 3/Day** on the **Mage + Archmage** (slice 819 — Counterspell + Shield, both dedicated slot-consuming reaction planners, gained `useFreeCast` per-day metering via a shared `resolvePerDayFreeCast` helper extracted across the three dedicated planners (Misty Step refactored onto it), pooled under `perDayPoolId: 'protective-magic'`). **Still open:** only **Dryad Tree Stride** — a teleport *movement* ability, not a spell at all (a different subsystem; tracked under `single-target-recharge`-style movement work, not the per-day spell machinery). |
