...
**Spellcasting (cast-time)**: `castSpell` (the general dispatcher; slice 787: pass an opt-in `aim: {x,y}` in feet on an area spell and the engine derives the covered creatures from the rasterizer + line of effect, ignoring `targetIds` — the per-target range gate is skipped since RAW range is to the origin; slice 849: pass `unwillingTargetIds` on a buff spell whose mechanic carries `unwillingSave` (Enlarge/Reduce) and each named target rolls that save vs the caster's spell DC — a success means "no effect", while unnamed targets are treated as willing and skip the save), `magicWeapon`, `elementalWeapon`, `counterspell`, `dispelMagic`, `identify`, `removeCurse` (slice 134: strips every condition tagged `category: 'curse'` on the touched target), `silentImage` + `majorImage` (slice 137: places an Illusion entity; consumers use `investigateIllusion` to roll Investigation against the baked DC), `clairvoyance` + `scrying` (slices 135 + 136: places a remote Sensor entity bound to caster concentration; Scrying composes the same primitive with a target WIS save), `arcaneEye` (slice 138: places a mobile Sensor with darkvision 30).

**Spellcasting (tick / trigger)**: `checkConcentration`, `expireSpellDurations`, `tickAura`, `tickMovementDamage`, `tickRecurring`, `tickRecurringSave`, `tickRecurringDamage`, `tickRecurringHeal`, `triggerTrap`. Consumers call these at the appropriate moments (per-turn ticks, on-movement, on-trigger). `tickRecurringSave` accepts an optional `advantage: true` for repeat saves RAW grants Advantage to (Hideous Laughter's damage-triggered re-save: roll it with the flag when the bearer takes damage, flat at end of turn). `tickRecurringDamage` / `tickRecurringHeal` are the no-save per-turn damage / heal mirrors driven by a condition's `recurringDamage` / `recurringHeal` metadata (the infernal-wound drip; Aura of Life's 0-HP revive via `recurringHeal { onlyAtZeroHp }`). `tickAura` ticks a concentration aura (Spirit Guardians) from the caster's concentration effect, a **non-concentration** one (Faithful Hound's bite, Grease's prone area) when passed an explicit `spellId` (slice 872), or a budgeted non-concentration aura (Guardian of Faith) when passed an `effectInstanceId` (slice 873) — the effect holds the cumulative-damage budget that decrements each tick until the aura vanishes.
...
- Spatial / movement: `terrainAt`, `movementCostFor`, `movementCostAt`, `chebyshevDistanceFeet`, `isInRangeFeet`, `hasLineOfSight`, `hasLineOfEffect`. The last two are corner-aware (slice 901): they treat the origin as a point and the target as a space (centre + four corners), so a target is blocked only when *every* line to it is blocked, with a seam rule that never lets a sightline leak through the corner of a solid wall. They remain a deliberate identity — the current terrain vocabulary has no blocker that stops sight but not effect (or vice versa).
- Ability checks: `computeAbilityCheck`, `computePassiveScore`.
- Movement speed: `getEffectiveSpeeds(input)` (all modes; `walk` always, non-walk modes when > 0) and `getEffectiveSpeed(input)` (walk only). Each folds the effect stack's `ModifySpeed` entries per RAW (add / set / multiply / matchWalkSpeed, zero-speed wins), then applies the Heavy-Armor Strength-requirement −10 (slice 799) and finally the over-capacity cap (slice 866): every mode's Speed is clamped to ≤ 5 ft when the carried inventory weight exceeds the size×STR×15 carry max (`computeEncumbrance(...).overCapacity`). The cap only lowers Speed; an empty inventory takes a no-weigh fast path.
- Jump distance (slice 863): `computeJumpDistances(input)` → `{ longJumpFeet, standingLongJumpFeet, highJumpFeet, standingHighJumpFeet }`. Long Jump = effective STR score, High Jump = `max(0, 3 + STR mod)`, each standing variant = half (rounded down). Reads effective STR (Gauntlets of Ogre Power lengthen it). RAW each foot of jump costs a foot of movement — the consumer spends Speed against the distance; the engine reports it only.
...
- `targetCanSeeAttacker?: boolean` (slices 278/886, `AttackIntent`) — Dodge LoS gate (default-apply) + the Unseen-Attacker Advantage arm (`false` → attacker gains Advantage; RAW "when a creature can't see you").
- `attackerCanSeeTarget?: boolean` (slice 886, `AttackIntent`) — the Unseen-Target Disadvantage (`false` → attacker rolls with Disadvantage vs a target it can't see). Opt-in; generalizes the Invisible condition to darkness/obscurement/Blinded; nets with the advantage arm (mutual blindness → straight roll).
...
- `coverByTargetId?: Record<string, CoverKind>` (slice 885, `CastSpellIntent`) — per-target Cover for a spell save's +2/+5 Dex bonus (Half / Three-Quarters). Opt-in: a target absent from the map (or an omitted map) gets no cover. Only Dex saves consume it; the Sacred Flame `saveIgnoresCover` RAW exception is engine-modeled.
...
The fixed vocabulary the engine reads to compute character state. 71 kinds (70 primitives + the `Custom` escape hatch); see `EFFECT_KINDS` in [src/schemas/effects.ts](../src/schemas/effects.ts) for the canonical list. Highlights:
...
validateAttunement(character, instance, content);       // -> { issues: string[], unverifiedCondition? } (slice 864: attunement is consumer-committed, so this is the pre-flight check — `issues` are machine-checkable blockers (item doesn't require attunement / already attuned / 3-slot limit full); `unverifiedCondition` is the free-form restriction ("by a Dwarf") the engine can't verify, for the consumer/DM to confirm)
```
