...
The fixed vocabulary the engine reads to compute character state. 70 kinds (69 primitives + the `Custom` escape hatch); see `EFFECT_KINDS` in [src/schemas/effects.ts](../src/schemas/effects.ts) for the canonical list. Highlights:
