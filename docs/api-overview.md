...
The fixed vocabulary the engine reads to compute character state. 52 kinds (51 primitives + the `Custom` escape hatch); see `EFFECT_KINDS` in [src/schemas/effects.ts](../src/schemas/effects.ts) for the canonical list. Highlights:
