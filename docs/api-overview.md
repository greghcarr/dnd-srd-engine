...
**Spellcasting (tick / trigger)**: `checkConcentration`, `expireSpellDurations`, `tickAura`, `tickMovementDamage`, `tickRecurring`, `tickRecurringSave`, `triggerTrap`. Consumers call these at the appropriate moments (per-turn ticks, on-movement, on-trigger). `tickRecurringSave` accepts an optional `advantage: true` for repeat saves RAW grants Advantage to (Hideous Laughter's damage-triggered re-save: roll it with the flag when the bearer takes damage, flat at end of turn).
