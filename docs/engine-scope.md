...
- **Carrying capacity & encumbrance**: `computeCarryingCapacity` derives the size-scaled carry max (STR × 15 × size factor; Goliath Powerful Build counts one size larger) plus the double push/drag/lift; `computeEncumbrance` reports the binary `overCapacity` (summed inventory weight > carry max). The one RAW consequence is applied automatically — over capacity caps every movement mode's effective Speed at ≤ 5 ft (slice 866) — and the character sheet surfaces the breakdown. `computeJumpDistances` (slice 863) reports Long/High jump from effective STR. (Item weights are authored on item definitions; the engine sums them from the inventory.)
...
- **Surprise.** Who is "caught unawares" at the start of combat (RAW 2024: Disadvantage on the Initiative roll) is a consumer call — the engine has no stealth/perception model. Pass `RollInitiativeIntent.surprisedCombatantIds` (slice 802) and the engine rolls those combatants' Initiative with Disadvantage; omit it and nobody is surprised.
...
- **In-game time of day / calendar.** `inGameTime.totalMinutes` is bumped only via `InGameTimeAdvanced` events your app emits. The engine doesn't auto-advance time as turns pass. One rule reads this clock: the SRD 16-hour **Long Rest cadence** ("wait at least 16 hours after finishing one before starting another"). It's **opt-in** — set `CampaignSettings.enforceLongRestCadence: true` and `planLongRest` rejects a participant's next Long Rest until 16 in-game hours have elapsed since its last (the engine records each rest's completion time against `inGameTime`; you own advancing the clock across the rest's own duration). Left off (the default), the engine enforces no cadence, since without a clock every rest would sit at the same minute.
...
- **Spell area target selection.** As of slice 786 the engine ships the canonical rasterizer: `engine.query.creaturesInSpellArea(...)` (and the pure `coveredCells`) compute who's in the cone/sphere/line/cube/cylinder/emanation from positions + line of effect, so consumers no longer hand-roll the geometry. Pass an opt-in `aim` (a point in feet) on the cast intent (slice 787) and the engine derives and enforces membership itself; or pass an explicit `targetIds: string[]` (and call `creaturesInSpellArea` yourself to get the canonical ids). Choosing a legal *aim placement* within the spell's range is still your app's job (see `consumer-populate-positions`).
...
| `targetCanSeeAttacker` | `boolean` | `AttackIntent`, `ResolveAttackInput` | default-apply for Dodge (undefined → true); opt-in for the Unseen-Attacker Advantage (only `false` grants it) | Dodge's `ImposeDisadvantageOnAttackers` **and** (slice 886) the general Unseen-Attacker **Advantage** (RAW "when a creature can't see you, you have Advantage"). Per-attacker. Supply `false` when the target cannot see this attacker → no Dodge disadvantage AND this attacker gains Advantage. |
| `attackerCanSeeTarget` | `boolean` | `AttackIntent`, `ResolveAttackInput` | opt-in (undefined → no change) | The Unseen-**Target** Disadvantage (RAW "when you make an attack roll against a target you can't see, you have Disadvantage"). Generalizes the Invisible condition to darkness / obscurement / Blinded. Supply `false` when the attacker can't see the target; `true`/omit when it can (incl. an attacker whose blindsight/truesight defeats the obscurement). |
...
| `attackerHasHostileAdjacent` | `boolean` | `AttackIntent`, `ResolveAttackInput` | default-apply (undefined → conservative any-adjacent geometry); engine falls back to "any non-incapacitated combatant within 5 ft" | Ranged Attacks in Close Combat disadvantage. RAW gates on a *hostile* creature within 5 ft, but the engine has no hostility model — a hostility-aware consumer answers the predicate directly (an archer next to a friendly cleric passes `false`); `undefined` keeps the conservative geometry. |
| `coverByTargetId` | `Record<string, CoverKind>` | `CastSpellIntent` | opt-in (undefined / absent target → no cover) | A spell save's Cover bonus (+2 Half / +5 Three-Quarters on **Dex** saves; other abilities ignore cover). Per-target, since an AoE's targets have different cover. The engine doesn't model positions, so the consumer supplies it. Sacred Flame's "no benefit from Cover" RAW exception is handled engine-side via the `saveIgnoresCover` mechanic flag. |
