...
Even within the L3 row, twelve subclass features ship as content stubs (`effects: []`, i.e. the name appears but no mechanical wiring): Circle of the Land's nature cantrip + Land's Aid + Circle Spells (3 entries), Oath of Devotion's Devotion Spells, Hunter's Lore + Hunter's Prey (ranger hunter), Fast Hands (rogue thief), Open Hand Technique (monk), Evocation Savant (evoker wizard), Dark One's Blessing (warlock fiend), Draconic Resilience HP (draconic sorcery), and Preserve Life (life domain). (Note: Evoker's Sculpt Spells is an L6 feature, not L3, matching SRD 5.2.1; and Fiend Patron Spells is wired via `GrantSpell`, not a stub. L6+ stubs are catalogued in the SRD audit's Layer 4.)
