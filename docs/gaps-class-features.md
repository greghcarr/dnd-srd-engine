...
Even within the L3 row, eight subclass features ship as content stubs (`effects: []`, i.e. the name appears but no mechanical wiring): Circle of the Land's nature cantrip + Land's Aid + Circle Spells (3 entries), Hunter's Lore (ranger hunter; narrative, knows a marked creature's defenses), Fast Hands (rogue thief), Open Hand Technique (monk), Dark One's Blessing (warlock fiend), and Preserve Life (life domain). (Slices 346-348 wired four previously-stubbed L3 features: Oath of Devotion's Devotion Spells, now an always-prepared `GrantSpell` progression across L3/L5/L9/L13/L17; Draconic Resilience HP, now `+sorcerer-level` HP max via `AddModifier`; Evoker's Evocation Savant, now an `OfferChoice oneOf:2` over the L1-2 evocation wizard spells; and Hunter's Prey (Colossus Slayer arm), an `OfferChoice` whose Colossus Slayer option is an `OnEvent` AttackRolled rider (`oncePer: turn`, +1d8) gated on the new `event.targetMissingHp` fact, with Horde Breaker left a deferred stub. Notes: Circle of the Land's "Bonus Cantrip" is a 2014-PHB feature, not in SRD 5.2.1 (the 2024 Circle of the Land L3 grants only Circle of the Land Spells + Land's Aid), kept as a content-authoring extra. Evoker's Sculpt Spells is an L6 feature, not L3, matching SRD 5.2.1; and Fiend Patron Spells is wired via `GrantSpell`, not a stub. L6+ stubs are catalogued in the SRD audit's Layer 4.)
