...
- **Effect-primitive vocabulary.** Features (class features, feats, magic item powers, conditions) are described via a fixed vocabulary of effect primitives (the authoritative list is `EFFECT_KINDS` in [src/schemas/effects.ts](src/schemas/effects.ts); currently 70 primitives plus the `Custom` escape hatch). Wild Shape, Polymorph, Wish, Simulacrum and a handful of others drop to code handlers.
...
**Alpha.** Architecturally complete and content-substantial. A comprehensive test suite (unit, golden + transcripts, property, audit, boundaries) backs the load-bearing invariants (event-sourcing, plan/commit, RNG capture, replay equivalence, branded IDs, effect primitives), which are locked and proven; the engine compiles and builds (ESM + CJS + `.d.ts`). The 48-probe RAW-compliance audit at [tests/audit/raw-compliance.test.ts](tests/audit/raw-compliance.test.ts), plus the SRD ground-truth conformance audits that compare derived values (AC, saves, spell DC, weapon damage, speeds) to the SRD text, pass in full; the [SRD coverage ledger](docs/srd-coverage-ledger.md) tracks per-rule verification (which rules are checked against the book vs author-asserted). **SRD 5.2.1 pack-presence is complete across every category** (spells, monsters, magic items, species, feats, backgrounds, conditions); what's still growing is mechanical wiring depth, not catalog coverage.

At a glance:

- **Engine architecture**: 100%. Locked.
- **Effect-primitive vocabulary**: the majority of the planned primitives are shipped (70 primitives + `Custom` escape hatch in `EFFECT_KINDS`); the queue of remaining primitives is enumerated in [docs/starter-pack-gaps.md](docs/starter-pack-gaps.md).
- **Classes / species / backgrounds / feats / conditions / spells / monsters / magic items**: SRD 5.2.1 catalog complete in every category. Spell mechanical wiring ~64% (217/339 wired, 68 narrative, 54 schema-only; the 12 non-SRD wired spells moved to the phb-2024-extras pack in slice 402); magic-item mechanical wiring roughly a third (~91/258, after single-base magic weapons/armor were re-modeled into the weapon/armor categories in slices 315-317); consumables roughly two thirds (~45/69).
