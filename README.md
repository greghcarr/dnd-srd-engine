...
- **Effect-primitive vocabulary.** Features (class features, feats, magic item powers, conditions) are described via a fixed vocabulary of effect primitives (the authoritative list is `EFFECT_KINDS` in [src/schemas/effects.ts](src/schemas/effects.ts); currently 64 primitives plus the `Custom` escape hatch). Wild Shape, Polymorph, Wish, Simulacrum and a handful of others drop to code handlers.
...
- **Effect-primitive vocabulary**: the majority of the planned primitives are shipped (64 primitives + `Custom` escape hatch in `EFFECT_KINDS`); the queue of remaining primitives is enumerated in [docs/starter-pack-gaps.md](docs/starter-pack-gaps.md).
