...
- **Prefer a guard.** The count audits above are the model (derive from source, fail until the doc matches). [tests/audit/doc-links.test.ts](tests/audit/doc-links.test.ts) (slice 432) guards every internal markdown link (resolved from the file's own dir, the way GitHub resolves them), so the link-rot class that used to need manual sweeps now fails CI. [tests/audit/coverage-ledger.test.ts](tests/audit/coverage-ledger.test.ts) pins the ledger's cited probe / boundary counts. When you add a new drift-prone citation a script could check, add it to a guard rather than relying on memory.
...
Still un-guarded by design and needing a manual eye on material change: mechanical-wiring percentages, prose feature lists, and cross-doc narrative currency (e.g. a roadmap's slice-by-slice history). A future high-value guard: typechecking the `ts` code examples in the front-door docs against the real public API, so a renamed/removed export breaks CI instead of shipping copy-paste-broken example code.
