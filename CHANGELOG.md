...
**Surfaced a real gap (tracked, not yet fixed):** a character's effective walk speed comes from `character.speedFeet`, but the content-free `createPC` convenience defaults it to 30 rather than deriving it from the species, so a **Goliath built via createPC reports walk 30, not 35.** The species *data* is correct (verified 🟢 above); what's missing is a content-aware creation path that applies it (the consumer must currently set `speedFeet`). Fixing it touches creation/derivation semantics with broad blast radius, so it's deliberately not bundled with this conformance test; tracked as a 🔴 row in the [coverage ledger](docs/srd-coverage-ledger.md). This is the arc paying for itself a second time (after slice 422's missing firearms): ground-truth comparison found a player-visible wrong number that author-asserted tests missed.
