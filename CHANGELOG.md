...
The `0.1.0-alpha.11` release block kept the full per-slice detail for slices 337-343 inline (unlike the alpha.10 block, which uses a cohort summary + archive pointer), and that detail was the single largest inline bulk in the live file. Per the doc-size playbook, it moved to [docs/changelog/archive-slices-337-343.md](docs/changelog/archive-slices-337-343.md); the release block keeps its release note + cohort highlights and gains a bold per-cohort pointer line matching the 329-336 entry. Also fixed two archive-index drifts: added the missing [archive-slices-352-353.md](docs/changelog/archive-slices-352-353.md) entry (slice 355 created the file but never indexed it) and the new 337-343 entry, both in slice-number order. No code or content change; doc-size audit green; live CHANGELOG dropped from ~56 KB to ~40 KB.
...
The live CHANGELOG reached ~58.5 KB (approaching the 60 KB single-Read ceiling) after the slice 354 entry. Per the doc-size playbook, the per-slice detail for slices 352-353 moved to [docs/changelog/archive-slices-352-353.md](docs/changelog/archive-slices-352-353.md); the live file keeps the most recent slice (354) inline plus the pointer below. Added the new file to the archive index. No code or content change; doc-size audit green.
