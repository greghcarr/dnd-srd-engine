...
Opens a reaction window between the attack roll and the damage so prevent-the-trigger reactions genuinely cancel a hit, completing the slice-749 reaction layer for the attack-roll window. Driver-side two-phase flow (plan the attack uncommitted → Shield/Cutting-Words cascade → commit the full attack, or the attack minus its damage chain when prevented), gated behind `reactions: 'auto'`; no engine change (it composes the existing `planShield` / `planCuttingWords` planners and their `preventedHit` contract). Shield (a Wizard/Sorcerer target, when +5 AC would miss) and Cutting Words (a Bard on the target's team) are wired; the slice rule keeps everything before the first damage event (`DamageRolled` *or* `DamageApplied` — rider damage can skip the roll) plus `WeaponLoaded`. Pure `shouldShield` / `shouldCuttingWords` predicates added to `src/ai/reactions.ts` (barrel-exported). Default `'none'` byte-identical (legacy inline Shield still fires there); under `'auto'` the cosmetic inline Shield is superseded by the real one and the slice-749 post-commit mitigation still layers on a hit that isn't prevented. Verified over 480 `'auto'` battles: 62 Shield preventions, 0 leaked damage, 0 replay mismatches. Known limits (driver-side vs a full engine two-phase): `AttackRolled.hit` stays true on a prevented hit, and a Cutting-Words miss doesn't synthesize on-miss effects. Counterspell / Countercharm / Protection deferred.
