...
const SPELL_EXPECTATIONS: Record<string, Expectation> = {
  ...
  'hallucinatory-terrain': { kind: 'skip', reason: 'NARRATIVE-ONLY by design (slice 892): a 24-hour illusion that makes natural terrain look/sound/smell like other terrain, with an Investigation check vs the spell DC to disbelieve. No mechanical combat effect — terrain appearance + the disbelieve adjudication are DM/consumer-owned, outside engine scope (engine-scope.md). Correctly mechanicalEffects: []; the L7 row l4-hallucinatory-terrain was closed as out-of-scope, not a deferred primitive.' },
  'locate-creature': { kind: 'skip', reason: 'NARRATIVE-ONLY by design (slice 892): senses the DIRECTION to a familiar creature within 1,000 ft (Concentration, 1 hour). The directional sense needs world geography + creature locations beyond the encounter grid — which the engine deliberately does not model (engine-scope.md) — so it is DM/consumer-adjudicated. Correctly mechanicalEffects: []; the L7 row l4-locate-creature was closed as out-of-scope, not a deferred primitive.' },
